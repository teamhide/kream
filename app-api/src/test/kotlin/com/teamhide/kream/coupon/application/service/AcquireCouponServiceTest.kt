...
internal class AcquireCouponServiceTest(
    ...
    Given("AcquireCouponService") {
        When("존재하지 않는 쿠폰 그룹 식별자로 쿠폰 획득을 요청하면") {
        ...
        When("발급된 쿠폰 개수가 총 개수와 같은 경우") {
            ...
            Then("쿠폰 획득을 요청하면 획득한 쿠폰은 회수 처리하고 예외가 발생한다") {
        ...
        When("쿠폰 그룹의 상태가 ACTIVATED가 아니라면") {
            val command = makeAcquireCouponCommand()
        ...
        When("발급된 쿠폰 개수가 총 개수와 작지만 획득에 실패한 경우") {
            val identifier = "uuid"
            val quantity = 100
