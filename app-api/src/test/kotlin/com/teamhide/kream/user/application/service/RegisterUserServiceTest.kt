...
internal class RegisterUserServiceTest(
    ...
    Given("RegisterUserService") {
        When("password1과 password2가 동일하지 않은 경우 회원가입을 요청하면") {
        ...
        When("동일한 이메일을 가진 유저가 존재하는 경우 회원가입을 요청하면") {
        ...
        When("동일한 이메일 또는 닉네임을 가진 유저가 존재하지 않는 경우 회원가입을 요청하면") {
            val command = makeRegisterUserCommand()
